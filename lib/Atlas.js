...
const constructResourcesTesting = (config, resourcesNameLoad) => {
  ...
  resources.forEach(__resources_route__ => {
    // Import Resource
    const ClassSchema = require(`${__model_route__}`);
    ...
    console.log(new ClassSchema().__proto__.constructor.name, resourcesNameLoad)
    console.log(verificationResource(new ClassSchema().__proto__.constructor.name, resourcesNameLoad));
...
const verificationResource = (className, resourceInvokeName) => {
  return className === resourceInvokeName
}
