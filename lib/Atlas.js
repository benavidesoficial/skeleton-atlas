...
class Atlas {
  ...
  [_connect]() {
    return new Promise((resolve, reject) => {
      mongoose.connect(this._config.mongodb, {
  ...
  // Define models
  [_isTesting]() {
    if (this._config.hasOwnProperty('atlasEnvironment')) {
      switch (this._config.atlasEnvironment) {
        case 'production':
          return false
        case 'testing':
          return true
...
const getMiddlewares = (Class) => {
  let middlewares = ['onBeforeValidate', 'onBeforeInsertMany', 'onBeforeCreate', 'onAfterCreate', 'onBeforeFindAndUpdate'];
